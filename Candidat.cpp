...
QSqlQueryModel* Candidat::getCandidatsDisponibles()
{
    ...
    QString queryStr = "SELECT ID_CANDIDAT, NOM_C, PRENOM_C, TELEPHONE_C, EMAIL, NV_PERMIS, PROGRESSION "
                       "FROM CANDIDATS "
                       "ORDER BY NOM_C, PRENOM_C";

    model->setQuery(queryStr);

    if (model->lastError().isValid()) {
        qDebug() << "❌ Erreur getCandidatsDisponibles:" << model->lastError().text();
    } else {
        qDebug() << "✅ Liste des candidats disponibles chargée -" << model->rowCount() << "candidats";
    }

    return model;
}

QSqlQueryModel* Candidat::getCandidatsDisponibles(const QDate& date, const QString& heureDebut, const QString& heureFin, int idSeanceExclue)
{
    QSqlQueryModel *model = new QSqlQueryModel();

    QString queryStr =
        "SELECT c.ID_CANDIDAT, c.NOM_C, c.PRENOM_C, c.TELEPHONE_C, c.EMAIL, c.NV_PERMIS, c.PROGRESSION "
        "FROM CANDIDATS c "
        "WHERE c.ID_CANDIDAT NOT IN ( "
        "    SELECT cs.ID_CANDIDAT "
        "    FROM CANDIDAT_SEANCE cs "
        "    JOIN SEANCES s ON cs.ID_SEANCE = s.ID_SEANCE "
        "    WHERE s.DATE_S = :date "
        "    AND ((s.HEURE_DEBUT_S BETWEEN :heureDebut AND :heureFin) "
        "         OR (s.HEURE_FIN_S BETWEEN :heureDebut AND :heureFin) "
        "         OR (:heureDebut BETWEEN s.HEURE_DEBUT_S AND s.HEURE_FIN_S)) ";

    if (idSeanceExclue != -1) {
        queryStr += " AND s.ID_SEANCE != :idSeanceExclue ";
    }

    queryStr += ") ORDER BY c.NOM_C, c.PRENOM_C";
    ...
    if (idSeanceExclue != -1) {
        query.bindValue(":idSeanceExclue", idSeanceExclue);
    }

    if (query.exec()) {
        model->setQuery(std::move(query)); // Use std::move to avoid the deprecation warning
        qDebug() << "✅ Candidats disponibles pour" << date.toString("dd/MM/yyyy")
                 << heureDebut << "-" << heureFin << ":" << model->rowCount() << "candidats";
    } else {
        qDebug() << "❌ Erreur getCandidatsDisponibles avec filtres:" << query.lastError().text();
        // Fallback to all candidates
        model->setQuery("SELECT ID_CANDIDAT, NOM_C, PRENOM_C, TELEPHONE_C, EMAIL, NV_PERMIS, PROGRESSION "
                        "FROM CANDIDATS ORDER BY NOM_C, PRENOM_C");
    }
