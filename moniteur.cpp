...
{
    QSqlQuery query;
    query.prepare("INSERT INTO MONITEURS (NOM_M, PRENOM_M, SPECIALITE_M, EXPERIENCE_M, DISPO_M, EMAIL_M, TEL_M, ID_SEANCE) "
                  "VALUES (:nom, :prenom, :specialite, :experience, :dispo, :email, :tel, :id_seance)");
...
{
    QSqlQuery query("SELECT CIN_M, NOM_M, PRENOM_M, EMAIL_M, TEL_M, SPECIALITE_M, DISPO_M, EXPERIENCE_M, ID_SEANCE FROM MONITEURS ORDER BY CIN_M ASC");

    table->setRowCount(0);
    table->setColumnCount(9);

    QStringList headers = {"CIN", "Nom", "Prénom", "Email", "Téléphone", "Spécialité", "Disponibilité", "Expérience", "ID Séance"};
...
bool Moniteur::modifier(QString cin_m, QString nom, QString prenom, QString email, QString tel,
                        QString dispo, QString specialite, int experience, int id_seance)
{
    QSqlQuery query;
    query.prepare("UPDATE MONITEURS SET NOM_M = :nom, PRENOM_M = :prenom, EMAIL_M = :email, "
                  "TEL_M = :tel, DISPO_M = :dispo, SPECIALITE_M = :specialite, "
                  "EXPERIENCE_M = :experience, ID_SEANCE = :id_seance "
                  "WHERE CIN_M = :cin");
...
bool Moniteur::modifier(QString cin_m, QString colonne, QVariant valeur)
{
    ...
    if (colonne == "CIN_M") {
        qDebug() << "⚠️ Tentative de modification du CIN interdite.";
        return false;
    }
