...
Seance::Seance(int id, QString type, QDate date, QString debut_h, QString fin_h,
               ...
               QString circuit_arrive_latitude, int id_moniteur)
{
    this->id = id;
    this->type = type;
    this->date = date;
...
QSqlQueryModel* Seance::afficher()
{
    ...
    QString queryStr = "SELECT ID_SEANCE, TYPE_S, TO_CHAR(DATE_S, 'DD/MM/YYYY'), HEURE_DEBUT_S, HEURE_FIN_S, "
                       "SALLE_S, CIRCUIT_DEP_S, LONGITUDE_S, CIRCUIT_DEP_LATITUDE, "
                       "CIRCUIT_ARRIVE_LONGITUDE, CIRCUIT_ARRIVE_LATIUDE, ID_MONITEUR "
    ...
    model->setHeaderData(11, Qt::Horizontal, QObject::tr("ID Moniteur"));
...
QSqlQueryModel* Seance::trier(QString colonne, QString ordre)
{
    ...
    QString queryString = "SELECT ID_SEANCE, TYPE_S, TO_CHAR(DATE_S, 'DD/MM/YYYY'), HEURE_DEBUT_S, HEURE_FIN_S, "
                          "SALLE_S, CIRCUIT_DEP_S, LONGITUDE_S, CIRCUIT_DEP_LATITUDE, "
                          "CIRCUIT_ARRIVE_LONGITUDE, CIRCUIT_ARRIVE_LATIUDE, ID_MONITEUR FROM SEANCES ";
...
bool Seance::exporterExcel()
{
    ...
    try {
        ...
        QString fileName = QFileDialog::getSaveFileName(nullptr, "Exporter les séances", "", "Fichiers Excel (*.xlsx)");
        if (!fileName.isEmpty()) {
            if (!fileName.endsWith(".xlsx", Qt::CaseInsensitive)) {
                fileName += ".xlsx";
            }
            workbook->dynamicCall("SaveAs(const QString&)", QDir::toNativeSeparators(fileName));
            QMessageBox::information(nullptr, "Succès", "Séances exportées vers Excel avec succès !");
        }
        ...
        QMessageBox::critical(nullptr, "Erreur", "Erreur lors de l'exportation Excel. Vérifiez que Excel est installé.");
        delete model;
        return false;
    }
}
