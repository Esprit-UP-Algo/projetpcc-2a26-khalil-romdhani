    ...
    , m_arduinoManager(nullptr)
{
    ...
    // Initialisation ArduinoManagerS
    m_arduinoManager = new ArduinoManagerS(this);
    // Connecter les signaux pour les messages
    connect(m_arduinoManager, &ArduinoManagerS::messageInfo, this, [this](const QString &msg) {
        QMessageBox::information(this, "Information", msg);
    });
    connect(m_arduinoManager, &ArduinoManagerS::messageWarning, this, [this](const QString &msg) {
        QMessageBox::warning(this, "Avertissement", msg);
    });
    connect(m_arduinoManager, &ArduinoManagerS::messageError, this, [this](const QString &msg) {
...
// ====================== ARDUINO COMPTE SÉANCES ======================
void MainWindow::on_compt_s_clicked()
{
    QString salleStr = ui->ard_s->text().trimmed();
    m_arduinoManager->compterEtEnvoyerSeances(salleStr);
...
// ====================== SÉANCE SLOTS (délégués à GestionSeanceUI) ======================
